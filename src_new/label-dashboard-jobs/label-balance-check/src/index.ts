...
interface SublabelBalanceSummary {
  sublabels: SublabelBalance[];
  total_amount: number;
  wallet_info?: {
    total_balance: number;
...
class LabelBalanceCheckService {
  ...
  /**
   * Fetch sublabels ready for payment from the System API
   *
   * The System API automatically filters for sublabels that are ready for payment:
   * - balance > 0 (positive balance)
   * - parent brand has at least one payment method
   */
  async fetchSublabelsReadyForPayment(): Promise<SublabelBalanceSummary> {
    console.log('Fetching sublabels ready for payment from System API (cross-brand)...');

    try {
      // Fetch all sublabels ready for payment across all brands using System API
      let allSublabels: SublabelBalance[] = [];
      let currentPage = 1;
      ...
      while (hasMorePages) {
        ...
        console.log(`Fetched page ${currentPage} of ${response.totalPages} (${response.results.length} sublabels ready for payment)`);

        hasMorePages = currentPage < response.totalPages;
        currentPage++;
      }

      console.log(`Retrieved ${allSublabels.length} total sublabels ready for payment from all brands`);

      // Group by parent brand for logging
      const parentBrandCounts = allSublabels.reduce((acc, sublabel) => {
        const parentBrandName = sublabel.parent_brand_name || 'Unknown';
        acc[parentBrandName] = (acc[parentBrandName] || 0) + 1;
        return acc;
      }, {} as Record<string, number>);

      console.log('Sublabels ready for payment by parent brand:');
      Object.entries(parentBrandCounts).forEach(([brandName, count]) => {
        console.log(`  - ${brandName}: ${count} sublabel${count === 1 ? '' : 's'}`);
      });

      // Calculate total amount
      const totalAmount = allSublabels.reduce((sum, sublabel) => sum + sublabel.balance, 0);

      console.log(`\nTotal: ${allSublabels.length} sublabels ready for payment across ${Object.keys(parentBrandCounts).length} parent brands`);
      ...
      console.error('Error fetching sublabel balances:', error.message);
      throw error;
    }
  }
   ...
   */
  private generateEmailHTML(summary: SublabelBalanceSummary): string {
    ...
    const sublabelRows = summary.sublabels
      .map(
        (sublabel) => `
        <tr>
          <td style="padding: 12px; border-bottom: 1px solid #e5e7eb; color: #374151;">
            ${sublabel.sublabel_name}
            <div style="font-size: 11px; color: #9ca3af; margin-top: 2px;">${sublabel.parent_brand_name}</div>
          </td>
          <td style="padding: 12px; border-bottom: 1px solid #e5e7eb; text-align: right; font-weight: 600; color: #059669;">
            ₱${sublabel.balance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </td>
        </tr>
      `
      )
...
<head>
  ...
  <title>Sublabel Balance Summary</title>
</head>
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f3f4f6;">
              ...
              <div style="background-color: #f0fdf4; border-left: 4px solid #059669; padding: 20px; border-radius: 6px; margin-bottom: 16px;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                  ...
                  <div style="background-color: #059669; color: #ffffff; padding: 8px 16px; border-radius: 20px; font-size: 14px; font-weight: 600;">
                    ${summary.sublabels.length} ${summary.sublabels.length === 1 ? 'Sublabel' : 'Sublabels'}
                  </div>
                </div>
              </div>
          ...
          <tr>
            <td style="padding: 24px 32px; background-color: #f9fafb; border-radius: 0 0 8px 8px; border-top: 1px solid #e5e7eb;">
              <p style="margin: 0; color: #6b7280; font-size: 12px; text-align: center;">
                This is an automated report generated by the Label Balance Check system.
              </p>
   ...
   */
  private generateEmailText(summary: SublabelBalanceSummary): string {
    const currentDate = new Date().toLocaleDateString('en-US', {
      weekday: 'long',
      year: 'numeric',
      month: 'long',
    ...
    if (summary.sublabels.length > 0) {
      text += `BREAKDOWN BY SUBLABEL:\n`;
      text += `-------------------------------------------\n\n`;

      summary.sublabels.forEach((sublabel) => {
        text += `${sublabel.sublabel_name} (${sublabel.parent_brand_name})\n`;
        text += `  Balance Due: ₱${sublabel.balance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}\n\n`;
      });
    } else {
      text += `No sublabels are currently ready for payment.\n\n`;
    }

    text += `===========================================\n\n`;
    text += `This is an automated report generated by the Label Balance Check system.\n`;
   ...
   */
  async sendEmailSummary(summary: SublabelBalanceSummary): Promise<void> {
    console.log(`Sending email summary to ${this.superadminEmail}...`);

    let subject = `Sublabel Balance Summary - ${summary.sublabels.length} Sublabels Ready for Payment (₱${summary.total_amount.toLocaleString('en-US', {
  ...
  async checkAndNotify(): Promise<{ sublabelCount: number; totalAmount: number }> {
    console.log('Starting sublabel balance check process...');
...
export const handler: Handler<ScheduledEvent, LambdaResponse> = async (event, context) => {
  console.log('Label Balance Check Lambda triggered', { event, context });

  try {
    const service = new LabelBalanceCheckService();
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({
        message: 'Label balance check failed',
        error: error.message,
      }),
    };
  }
