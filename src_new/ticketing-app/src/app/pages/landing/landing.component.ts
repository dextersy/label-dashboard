...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterLink, ShareModalComponent, AudienceAuthModalComponent, InviteFriendModalComponent],
  template: `
    <!-- Nav -->
      ...
      <div class="max-w-5xl mx-auto px-4 sm:px-6 flex items-center justify-between h-12">
        ...
        <div class="flex items-center gap-4">
          ...
          @if (isAudienceLoggedIn()) {
            <button type="button" (click)="audienceInviteModalOpen.set(true)"
              class="hidden sm:flex items-center gap-1.5 px-3 py-1.5 border border-white/15 text-[10px] font-mono text-white/50 uppercase tracking-wider hover:border-yellow-400/50 hover:text-yellow-400 transition-colors">
            ...
            <div class="relative">
              <button (click)="audienceMenuOpen.set(!audienceMenuOpen())"
                ...
                @if (audiencePhotoUrl()) {
                  <img [src]="audiencePhotoUrl()" alt="Profile" class="w-full h-full object-cover">
                } @else {
              ...
              @if (audienceMenuOpen()) {
                <div class="absolute right-0 top-full mt-2 w-44 bg-black border border-white/20 shadow-xl z-50">
                  ...
                  <a routerLink="/my-profile" (click)="audienceMenuOpen.set(false)"
                    class="flex items-center gap-2 px-4 py-2.5 text-xs font-mono text-white/60 hover:text-white hover:bg-white/5 uppercase tracking-wider transition-colors">
                    Edit Profile
                  </a>
                  <button type="button" (click)="audienceInviteModalOpen.set(true); audienceMenuOpen.set(false)"
                    class="w-full flex items-center gap-2 px-4 py-2.5 text-xs font-mono text-white/60 hover:text-white hover:bg-white/5 uppercase tracking-wider transition-colors">
                    Invite a Friend
                  </button>
    ...
    <!-- Audience invite modal -->
    @if (audienceInviteModalOpen()) {
      <app-invite-friend-modal
        [referralCode]="audienceReferralCode()"
        (close)="audienceInviteModalOpen.set(false)">
      </app-invite-friend-modal>
    }
...
export class LandingComponent implements OnInit, OnDestroy {
  ...
  audienceInviteModalOpen = signal(false);
  ...
  audiencePhotoUrl = () => this.audienceAuth.getUser()?.profile_photo_url || null;
  audienceReferralCode = () => this.audienceAuth.getUser()?.referral_code || '';
  audienceLogout(): void {
