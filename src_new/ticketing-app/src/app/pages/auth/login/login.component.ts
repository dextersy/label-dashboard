...
export class LoginComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Handle OAuth errors (Google or Facebook)
    const errorParam = params.get('error') || params.get('audience_error');
    if (errorParam) {
      if (errorParam === 'facebook_no_email') {
        this.error.set('Your Facebook account has no verified email. Please sign in with email and password.');
      } else if (errorParam.startsWith('facebook_')) {
        this.error.set('Facebook sign-in failed. Please try again.');
      } else {
    ...
    if (codeParam && this.mode() !== 'organizer') {
      ...
      this.http.post<any>(`${environment.apiUrl}/auth/audience/${provider}/exchange`, { code: codeParam })
        .subscribe({
          next: (res) => {
          ...
          error: () => {
            this.loginLoading.set(false);
            this.error.set(`${providerLabel} sign-in failed. Please try again.`);
          }
        });
      return;
    }
