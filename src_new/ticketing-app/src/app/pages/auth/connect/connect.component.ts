            ...
            <div class="space-y-2 mb-4">
              @if (googleAuthEnabled) {
                <button type="button" (click)="googleSignIn()"
                  class="flex items-center justify-center gap-3 w-full py-2.5 px-4 border border-gray-300 bg-white hover:bg-gray-50 text-gray-700 text-sm font-medium cursor-pointer transition-colors">
              ...
              @if (facebookAuthEnabled) {
                <button type="button" (click)="facebookSignIn()"
                  class="flex items-center justify-center gap-3 w-full py-2.5 px-4 border border-gray-300 bg-white hover:bg-gray-50 text-gray-700 text-sm font-medium cursor-pointer transition-colors">
...
export class ConnectComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Handle OAuth return: ?audience_code=<code>&audience_provider=<google|facebook>
    ...
    if (audienceCode) {
      ...
      this.http.post<any>(`${environment.apiUrl}/auth/audience/${provider}/exchange`, { code: audienceCode })
        .subscribe({
          next: (res) => { this.exchanging.set(false); this.notifyAndClose(res); },
          error: () => { this.exchanging.set(false); this.error.set(`${providerLabel} sign-in failed. Please try again.`); }
        });
      return;
    }
  ...
  private startOAuth(provider: string): void {
    const providerUrl = `${environment.apiUrl}/auth/audience/${provider}`;
    if (window.opener && this.returnTo) {
      // Redirect the main (parent) window to the OAuth provider directly, using the return_to
      // URL as the landing page after auth. This avoids the COOP problem entirely.
      window.opener.location.href = `${providerUrl}?return_to=${encodeURIComponent(this.returnTo)}`;
      window.close();
    } else {
      // No opener — do OAuth in this window.
      sessionStorage.setItem('ys_connect_return_to', this.returnTo);
      window.location.href = `${providerUrl}?return_to=${encodeURIComponent(window.location.href)}`;
    }
  }
